  ...
  context "Format" do
    ...
    context "Csv" do
      it "reads from a .csv file" do
        #puts Parser::Format::Csv.read_file('lib/assets/order_1.csv')
        expect(Parser::Format::Csv.read_file(@csv_files.first).blank?).to eql(false)
      end

      it "reads from multiple .csv files" do
        expect(Parser::Format::Csv.read_files(@csv_files).blank?).to eql(false)
      end
    end

    context "Txt" do
      it "reads from a .txt file" do
        expect(Parser::Format::Txt.read_file(@txt_file).blank?).to eql(false)
      end
      it 'hash array is correctly formatted' do
        returned_keys = Parser::Format::Txt.read_file(@txt_file).first.keys
